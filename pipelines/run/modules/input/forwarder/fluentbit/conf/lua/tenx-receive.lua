-- 10x 'receive' lua config --

-- this script loaded by tenx-receive.conf to configure the 10x sub-process and event processing
...
-- Record processing
	...
	-- In the 10x 'receive' configuration, events are handled and possibly dropped by
	-- the 10x pipeline, so this is true
...
-- Launching 10x
	...
	-- Define 10x arguments for running in 'receive' mode
	tenx_run_args = "@run/input/forwarder/fluentbit/receive/config.yaml @apps/receiver"
