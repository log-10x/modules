...
Defaults to TCP `127.0.0.1:24225` so it pairs with the [input](../input/) listening on `:24224`. Override the destination with `outputForwardHostAddress` / `outputForwardHostPort`, or `outputForwardAddress` for a Unix socket on Linux/macOS.
