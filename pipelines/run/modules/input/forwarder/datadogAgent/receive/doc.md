...
Read events from application logs to transform into typed [TenXObjects](https://doc.log10x.com/api/js/#TenXObject) and filter using local/centralized [receiver](https://doc.log10x.com/run/output/receive) policies, before the Datadog Agent ships them to Datadog. This module is a component of the [Receiver](https://doc.log10x.com/apps/receiver/) app.
...
This [module](https://doc.log10x.com/engine/module/) configures a file relay where Fluent Bit reads application logs, passes them through a 10x [sidecar process](https://doc.log10x.com/engine/launcher/sidecar) for filtering, then writes only the filtered events to a folder that the Datadog Agent monitors. Noisy/repetitive events are dropped before reaching Datadog.

### :material-download-outline: Install

=== ":material-laptop: Nix/OSX"

    See the [Quickstart](#quickstart) below or the Log10x Receiver [run instructions](https://doc.log10x.com/apps/receiver/run/#datadog-agent)
...
??? tenx-run "Step 4: Run"
    ...
    Compare the line counts in Folder A vs Folder B to see filtering in action.
