...
Read events from OpenTelemetry Collector and filter them using local/centralized [receiver](https://doc.log10x.com/run/output/receive) policy. This module is a component of the [Receiver](https://doc.log10x.com/apps/receiver/) app.
...
This [module](https://doc.log10x.com/engine/module/) configures a Unix socket input/output pair that receives syslog events from OpenTelemetry Collector, applies filter policy, and returns filtered events via Forward protocol. The sidecar relays received events back to OTel Collector to ship to outputs (e.g., Splunk, S3).

### :material-download-outline: Install

=== ":material-laptop: Nix/Win/OSX"

    See the Log10x Receiver OTel Collector [run instructions](https://doc.log10x.com/apps/receiver/run/#otel-collector)

=== ":material-kubernetes: k8s"

    Deploy to k8s via [Helm](https://helm.sh/)

    See the Log10x Receiver OTel Collector [deployment instructions](https://doc.log10x.com/apps/receiver/deploy/#otel-collector)
