...
Read events from a Logstash forwarder to transform them into typed [TenXObjects](https://doc.log10x.com/api/js/#TenXObject) to filter using local/centralized [receiver](https://doc.log10x.com/run/output/receive) policy. This module is a component of the [Receiver](https://doc.log10x.com/apps/receiver/) app.
...
This [module](https://doc.log10x.com/engine/module/) configures a Logstash [pipe output](https://www.elastic.co/guide/en/logstash/current/plugins-outputs-pipe.html) plugin and Unix/TCP input plugin. The Logstash output plugin launches a 10x [sidecar process](https://doc.log10x.com/engine/launcher/sidecar) and pipes events to it to receive using a local/centralized policy. The sidecar relays received events back to Logstash Unix/TCP input plugin to ship to output (e.g., ElasticSearch).

### :material-download-outline: Install

=== ":material-laptop: Nix/Win/OSX"

    See the Log10x Receiver Logstash [run instructions](https://doc.log10x.com/apps/receiver/run/#logstash)
