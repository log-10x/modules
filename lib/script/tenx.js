...
export class TenXObject extends TenXBaseObject {
     ...
     *
     * `drop()` can be called from a constructor or from a getter/method invoked later by a
     * {@link https://doc.log10x.com/run/transform/group/#groupfilters|groupFilter} or
     * {@link https://doc.log10x.com/run/output/filter|output filter}. The two have different runtime cost:
     *
     * - **Constructor (recommended fast path).** The decision is made before the event is committed
     *   to the pipeline. The engine short-circuits at the lexer: the event never reaches subsequent
     *   transform, group, aggregate, encode or output stages, and no further CPU is spent on it.
     *   Use this whenever the drop predicate can be resolved from intrinsic or extracted fields
     *   already available at construction time.
     *
     * - **Later (getter or method).** By the time a filter or output-context call runs, the event has
     *   already been materialized and has flowed through earlier stages. The `dropped` flag is set on
     *   the instance and honored by the {@link https://doc.log10x.com/run/output|output} stage so the
     *   event is not serialized, but earlier {@link https://doc.log10x.com/run/aggregate|aggregator}
     *   stages have already counted it. Reserve this for decisions that genuinely depend on
     *   post-construction context such as {@link TenXObject#outputType|outputType()} or
     *   {@link TenXObject#outputPath|outputPath()}.
     *
     * @example
