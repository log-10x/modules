...
export class TenXString {
    ...
    */
    ...
    /**
     * Returns an 11-char URL-safe identifier: the xxHash64 of the UTF-8 bytes
     * of 'value', serialized big-endian and encoded base64url without padding.
     *
     * Unlike {@link TenXString.hash} (a reversible base-85 encoding used for
     * internal template identity), this is a one-way, portable pattern key
     * intended to leave the pipeline: it reproduces byte-for-byte outside the
     * engine and is safe to use directly in SIEM queries and forwarder
     * filters. Populates the field named by the `symbolMessageHashField`
     * option.
     *
     * @example
     *  this.patternKey = TenXString.patternHash(this.symbolSequence("log,exec", "log", 0));
     *
     * @param {string} value Value to hash
     *
     * @return {string}      11-char base64url identifier, or null if value is null
     */
    static patternHash(value) {
        return TenXEngine.invoke(value);
    }
